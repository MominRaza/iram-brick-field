<?php
    ...
    $q = "select *from transaction, user, works where transaction.userid = user.userid and user.workid = works.workid and type = 1 and date = '$date'";
    $resultsi = mysqli_query($conn, $q);
    $r = "select *from transaction, user, works where transaction.userid = user.userid and user.workid = works.workid and type = 2 and date = '$date'";
    $resultsd = mysqli_query($conn, $r);
    $p = "select *from transaction, user, works where transaction.userid = user.userid and user.workid = works.workid and type = 3 and date = '$date'";
...
<body>
    ...
    <?php if($date == date('Y-m-d') && $ttt == null):?>
        ...
        <a href="./add/add-transactiontotal.php?previus=<?=$previusamount;?>&income=<?=$totalincome;?>&pakad=<?=$totalpakad;?>&paid=<?=$totalpaid;?>" class="option"><i class="material-icons">save</i>Save</a>
    <?php endif;?>
